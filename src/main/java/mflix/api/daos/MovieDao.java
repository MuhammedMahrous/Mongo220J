...
public class MovieDao extends AbstractMFlixDao {
     ...
     */
    private boolean
    validIdValue(String movieId) {
        boolean isValid = false;
      try {
          isValid = ObjectId.isValid(movieId);
      }
    ...
    public Document getMovie(String movieId) {
        ...
        pipeline.add(match);
        List<Bson> commentsPipeline = Arrays.asList(new Document("$lookup",
        ...
        pipeline.addAll(commentsPipeline);
    ...
    public List<Document> getMoviesByCountry(String... country) {
        ...
        List<Document> movies = new ArrayList<>();
        movies = moviesCollection.find(queryFilter).into(movies);
        return movies;
    }
